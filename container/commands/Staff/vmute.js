...
module.exports = class Vmute extends Command {
  constructor(...args) {
    super(...args, {
        name: "vmute",
        description: "Penalize any member in voice channels.",
        userRequiredRoles: settings.other.vmuterRole,
        category: "Staff",
        ...
        args: [{
          ...
          choices: [{
            ...
            name: "Using a voice changer. (15 dakika.)",
            value: "0007"
          }, {
            name: "Turning on bass music and trolling people. (1 saat.)",
            value: "0008"
          }, {
            name: "Other. (15 dakika.)",
            value: "0006"
          }]
        }, {
          name: "evidence",
  ...
  async run({client, interaction, member, message, guild, channel, respond, edit, args}) {
        ...
        if (member.user.id === target.user.id) return respond({
          content: "Kendin hakkında cezalandırma işlemi **uygulayamazsın**.",
          ephemeral: true
        })
        ...
        let check = await con.promise().query(`SELECT * FROM Penalties WHERE member_id = '${args[0]}' AND punishment_type = 'TEMP-VMUTE' AND is_ended = 0`)
        ...
        con.query(`INSERT INTO Penalties (member_id, punishment_type, punishment_reason, end_of_punishment, staff_id, extras) VALUES ('${args[0]}', 'TEMP-VMUTE', '${mutePunishReasonConverter(args[1])}', '${epoch_flake}', '${member.user.id}', '${args[2]}')`)
        target.voice.setMute(true, `VMuted! (By ${member.user.tag}.)`)

        con.query(`SELECT * FROM TotalPenalties WHERE member_id = '${args[0]}'`, async (err, rows) => {
          if (err) throw err; 
          if (rows.length < 1) {
            con.query(`INSERT INTO TotalPenalties (member_id, temp_vmute, total_points) VALUES ('${args[0]}', 1, ${generalPunishPointConverter(args[1])})`)
          } else {
            con.query(`UPDATE TotalPenalties SET temp_vmute = ${rows[0].temp_vmute} + 1 WHERE member_id = '${args[0]}'`)
        ...
        .setDescription(`**${target.user.tag}** member was **v-muted** by **${member.user.tag}** due to \`${mutePunishReasonConverter(args[1])}\` (\`#${punishment_number[0].length}\`)`)
        .setColor("GREYPLE")
  
        let log_embed = new MessageEmbed()
        .setAuthor(target.user.tag, target.user.avatarURL({dynamic: true}))
        .setColor("DARK_GREEN")
        .setDescription(`**${target.user.tag}** member was **v-muted** by **${member.user.tag}** due to \`${mutePunishReasonConverter(args[1])}\`.
